...
const Task = ({ taskObj, onComplete }) => {
  ...
  return (
    <div className="p-6 bg-[#fff] rounded-[5px] leading-normal mt-4 shadow-[0_4px_5px_0_rgb(0,0,0,0.1)]">
      ...
      <div className="text-xs pt-1">son teslim: <span className={`text-xs p-1 rounded-sm ${(distanceInDays <= 3) ? "bg-[#ffd9d4]" : "bg-[#d4d7ff]"}`}>{DistanceToNow}</span></div>
      ...
      {onComplete && <button className="text-sm block p-[8px_12px] ml-auto bg-[#fecc91] shadow-[0_4px_5px_0_rgb(0,0,0,0.05) rounded-[3px] border-0 cursor-pointer" onClick={() => onComplete(taskObj.id)}>Tamamlandı</button>}
    </div>
  );
};
